...
public class ReverseTimestampResolver extends CellResolver implements Resolver
{
    ...
    protected ReverseTimestampResolver()
    {
        className = "org.apache.cassandra.db.resolvers.ReverseTimestampResolver";
    }
    ...
    {
        logger.warn(String.format("ReverseTimestampResolver::writeTombstone(%s, %s, %s, %s)", writer, column, timestamp, localDeletionTime));
    ...
    {
        logger.warn(String.format("ReverseTimestampResolver::diff(%s, %s)", merged, cell));
        ...
        logger.warn(String.format("merged.livenessInfo().supersedes(cell.livenessInfo())? %s",merged.livenessInfo().supersedes(cell.livenessInfo()) ));
    ...
    {
        logger.warn(String.format("ReverseTimestampResolver::reconcile(clustering %s, existing %s, update %s, deletion  %s, writer  %s, nowInSec %s, indexUpdater %s", clustering, existing, update, deletion, writer, nowInSec, indexUpdater));
        logger.warn(String.format("Existing: has value %s , local deletion time: %s , local deletion time %s , is tombstone? %s, is live? %s ",
                existing.value().toString(),
                existing.livenessInfo().hasLocalDeletionTime() ,
                existing.livenessInfo().localDeletionTime(),
                existing.isTombstone(),
                existing.isLive(nowInSec)));

        logger.warn(String.format("Update: has  value %s, local deletion time: %s , local deletion time %s , is tombstone? %s, is live? %s ",
                update.value().toString(),
                update.livenessInfo().hasLocalDeletionTime() ,
                update.livenessInfo().localDeletionTime(),
                update.isTombstone(),
                update.isLive(nowInSec)));

        logger.warn(String.format("Existing has timestamp? %s", existing.livenessInfo().hasTimestamp()));


        if (existing == null || deletion.deletes(existing.livenessInfo()))
        {
            logger.warn("existing == null || deletion.deletes(existing.livenessInfo()), returning Long.MAX_VALUE");
        ...
        if (update == null || deletion.deletes(update.livenessInfo()))
        {
            logger.warn("update == null || deletion.deletes(update.livenessInfo(), writing to existing, returning Long.MAX_VALUE");
            existing.writeTo(writer);
            return Long.MAX_VALUE;
        }

        logger.warn(String.format("calling reconcile(%s, %s, %s)", existing, update, nowInSec));
        logger.warn(String.format("Existing cell delete? %s , update cell is a delete? %s", existing.livenessInfo().localDeletionTime(), update.livenessInfo().localDeletionTime()));
        ...
        logger.warn(String.format("returning Math.abs(%s - %s) = %s", existing.livenessInfo().timestamp(), update.livenessInfo().timestamp(), Math.abs(existing.livenessInfo().timestamp() - update.livenessInfo().timestamp())));
    ...
    {
        logger.warn(String.format("ReverseTimestampResolver::reconcile(%s, %s, %s)", c1, c2, nowInSec));
        if (c1 == null)
    ...
    public long reconcileComplex(Clustering clustering,
                                        ...
                                        SecondaryIndexManager.Updater indexUpdater)
    {
        logger.warn(String.format("ReverseTimestampResolver::reconcileComplex(%s, %s, %s, %s, %s, %s, %s, %s)", clustering, column, existing, update, deletion, writer, nowInSec, indexUpdater));
    ...
    {
        logger.warn(String.format("ReverseTimestampResolver::getNext(%s)", iterator));
    ...
    {
        return "ReverseTimestampResolver("+className+")";
    }
