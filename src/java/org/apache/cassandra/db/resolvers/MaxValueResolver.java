...
public class MaxValueResolver extends CellResolver implements Resolver
{
    private final String className;

    public static final CounterContext counterContextManager = CounterContext.instance();
    private static final Logger logger = LoggerFactory.getLogger(MaxValueResolver.class);

    protected MaxValueResolver()
    {
        className = "org.apache.cassandra.db.resolvers.MaxValueResolver";
    }
    ...
    {
        logger.warn("MaxValueResolver::writeTombstone()");
    ...
    {
        // logger.warn("MaxValueResolver::diff(cell merged, Cell cell");
    ...
    {
        logger.warn(String.format("MaxValueResolver::reconcile(clustering %s, existing %s, update %s, deletion  %s, writer  %s, nowInSec %s, indexUpdater %s", clustering, existing, update, deletion, writer, nowInSec, indexUpdater));
        ...
        logger.warn("MaxValueResolver::reconcile ");
        if (existing == null || deletion.deletes(existing.livenessInfo()))
        {
            logger.warn("MaxValueResolver::reconcile 163");
        ...
        if (update == null || deletion.deletes(update.livenessInfo()))
        {
            logger.warn("update == null? " + (update == null) );
            logger.warn("deletion.deletes(update.livenessInfo()) ? " + deletion.deletes(update.livenessInfo()));
            logger.warn("MaxValueResolver::reconcile 176, write to " + writer.toString() + " and return Long.MAX_VALUE");
    ...
    {
        logger.warn(String.format("MaxValueResolver::reconcile(%s, %s, %s)", c1, c2, nowInSec));
        if (c1 == null)
        ...
        {
            logger.warn("MaxValueResolver::reconcile c1 isCounterCell or c2 isCounterCell ");
        ...
        if ( !c1Live || !c2Live )
        {
            logger.warn(String.format("tombstone detected: c1Live? %s , c2Live? %s", c1Live, c2Live));
            // One or more tombstones, pick the highest timestamp
            long ts1 = c1.livenessInfo().timestamp(), ts2 = c2.livenessInfo().timestamp();
            if (ts1 != ts2)
                return ts1 < ts2 ? c2 : c1;

            logger.warn(String.format(" %s > %s ? %s : %s", c1.livenessInfo().localDeletionTime(), c2.livenessInfo().localDeletionTime(), c2, c1));
    ...
    public long reconcileComplex(Clustering clustering,
                                        ...
                                        SecondaryIndexManager.Updater indexUpdater)
    {
        logger.warn("MaxValueResolver::reconcileComplex()");
    ...
    {
        logger.warn(String.format("MaxValueResolver.getNext(%s)", iterator));
    ...
    {
        return "MaxValueResolver("+className+")";
    }
