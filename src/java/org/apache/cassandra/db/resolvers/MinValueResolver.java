...
public class MinValueResolver extends CellResolver implements Resolver
{
    private final String className;

    public static final CounterContext counterContextManager = CounterContext.instance();
    private static final Logger logger = LoggerFactory.getLogger(MinValueResolver.class);

    protected MinValueResolver()
    {
        className = "org.apache.cassandra.db.resolvers.MinValueResolver";
    }
    ...
    {
        logger.warn("MinValueResolver::writeTombstone()");
    ...
    {
        logger.warn("MinValueResolver::diff(cell merged, Cell cell");
    ...
    {
        logger.warn(String.format("MinValueResolver::reconcile(clustering %s, existing %s, update %s, deletion  %s, writer  %s, nowInSec %s, indexUpdater %s", clustering, existing, update, deletion, writer, nowInSec, indexUpdater));
        ...
        logger.warn("MinValueResolver::reconcile ");
    ...
    {
        logger.warn(String.format("MinValueResolver::reconcile(%s, %s, %s)", c1, c2, nowInSec));
        if (c1 == null)
    ...
    public long reconcileComplex(Clustering clustering,
                                        ...
                                        SecondaryIndexManager.Updater indexUpdater)
    {
        logger.warn("MinValueResolver::reconcileComplex()");
    ...
    {
        logger.warn(String.format("MinValueResolver.getNext(%s)", iterator));
    ...
    {
        return "MinValueResolver("+className+")";
    }
