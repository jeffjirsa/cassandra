...
public class BufferExpiringCounterUpdateCell extends BufferCell implements ExpiringCounterUpdateCell
{
    public BufferExpiringCounterUpdateCell(CellName name, long value, long timestamp)
    {
        this(name, ByteBufferUtil.bytes(value), timestamp);
    }

    public BufferExpiringCounterUpdateCell(CellName name, ByteBuffer value, long timestamp)
    {
    ...
    public Cell diff(Cell cell)
    {
        ...
        throw new UnsupportedOperationException("This operation is unsupported on ExpiringCounterUpdateCell.");
    }
    ...
    public Cell reconcile(Cell cell)
    {
        ...
        assert cell instanceof ExpiringCounterUpdateCell : "Wrong class type.";

        // The only time this could happen is if a batch ships two increments for the same cell. Hence we simply sum the deltas.
        return new BufferExpiringCounterUpdateCell(name, delta() + ((ExpiringCounterUpdateCell) cell).delta(), Math.max(timestamp, cell.timestamp()));
    }
