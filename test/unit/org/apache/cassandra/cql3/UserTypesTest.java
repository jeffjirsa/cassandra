...
public class UserTypesTest extends CQLTester
{
    ...
    {
        String utSet1 = createType("CREATE TYPE %s (a int)");

        createTable("CREATE TABLE %s (x int PRIMARY KEY, y frozen < set < " + KEYSPACE + "." + utSet1 + " > >)");

        execute("INSERT INTO %s (x, y) VALUES(1, {1} )");
        assertRows(execute("SELECT * FROM %s"), row(1, set(userType(1))));
        flush();

        execute("ALTER TYPE " + KEYSPACE + "."+ utSet1 + " ADD b int");
    ...
    public void testAlteringUserTypeNestedWithinList() throws Throwable
    {
        ...
        createTable("CREATE TABLE %s (x int PRIMARY KEY, y frozen < list < " + KEYSPACE + "." + utSet1 + " > >)");
